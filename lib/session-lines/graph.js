 ...
 *   subLinks: Object<string, Array<{score: string, returnHref: (string|null)}>>,
...
function buildGraph(frames) {
  ...
  for (const frame of frames) {
    ...
    if (attrs.subLinks && attrs.subLinks.length > 0) {
      graph.subLinks[name] = attrs.subLinks.map((l) => ({
        score: l.score,
        returnHref: l.href ?? null,
      }));
    }
