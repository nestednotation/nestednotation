...
// not shape it (owner: "a rider is a spectator, and a spectator can't affect
// the session in any way, and should only be able to view active
...
/**
 * May this connection act on `sessionId` at all?
 *
 * A socket is tagged with its room, its staff standing and its line inside the
 * MSG_PING branch — so before the handshake every one of those fields is
 * UNDEFINED. The spectator rule that used to stand here reads `isStaff ===
 * false`, which undefined is not, so a socket that opened and sent a TAP first
 * was neither refused as a rider nor admitted as a performer but simply
 * unchecked: it fell through to the line lookup, which resolves an unbound
 * connection to the room's first active line, and moved it. Knowing a session
 * id and a frame index was the whole of the credential.
 *
 * Stated positively instead, and asked of the room the MESSAGE names: a
 * connection bound to room A must not be able to act on room B by putting B's
 * id in a payload.
 */
function isBoundToSession(conn, sessionId) {
  return !!conn && !!conn.sessionId && conn.sessionId === sessionId;
}

/**
 * …and may it act as a PERFORMER on it — the authorization a tap needs?
 *
 * Three things, all required:
 *   - the connection is bound to this room (above);
 *   - it authenticated as staff at the handshake AND the message in hand
 *     carries valid credentials too (`credentialsValid`), so a socket cannot
 *     change what it claims to be between messages;
 *   - it is not an observation tool. A /map tab authenticates with the admin
 *     password and so passes every other check there is, but it renders no
 *     frames and offers no links: a tap arriving on one is not a performance
 *     gesture at all.
 */
function canPerform(conn, sessionId, credentialsValid) {
  return (
    isBoundToSession(conn, sessionId) &&
    conn.isStaff === true &&
...
/**
 * Where a SPECTATOR should watch from (owner). Unlike a performer, a rider is
 * never assigned to an inactive line: it cannot tap, so a dormant line it
 * landed on would never wake, and a retired one shows nothing at all — "a
 * spectator should only be able to view active lines/nodes".
 *
...
/**
 * The dormant line the room most recently LEFT — the one to revive when a
 * device joins a room with no active line at all.
 *
 * Asked only then. With an active line, #13's revival and balancing rules
 * decide; with none there is no position of record to fast-forward to, and
 * the lowest-numbered dormant line is an accident of numbering: a split's
 * first branch keeps the parent's number even when nobody takes it, so a room
 * that finished on END as L3 left an empty L0 on a mid-score fork, and the
 * next joiner woke THAT one and was shown the middle of the score.
 *
 * `dormantAt` is stamped only by attrition, so a line nobody was ever on (an
 * empty split branch, an unclaimed merge-undo route) never outranks one the
 * room actually played. Null when no dormant line carries a stamp (a state
 * file from before the field existed) — the caller falls back to
 * `smallestLineId`.
 *
 * @param {Array<{id:string,status?:string,dormantAt?:number|null}>} lines
 * @returns {string|null}
 */
function lastVacatedLineId(lines) {
  let best = null;
  for (const line of lines || []) {
    if (!line || line.status !== "dormant") continue;
    if (typeof line.dormantAt !== "number") continue;
    if (
      best === null ||
      line.dormantAt > best.dormantAt ||
      (line.dormantAt === best.dormantAt &&
        lineIdNum(line.id) < lineIdNum(best.id))
    ) {
      best = line;
    }
  }
