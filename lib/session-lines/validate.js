 ...
 *   - split N is a whole number, == href count, and >= 2
 *   - runtime-read attributes use double quotes (the build ignores single)
 ...
 *   - no two links on one frame share a target when either is a dive (the
 *     runtime routes by target frame and cannot tell them apart)
 ...
 *     group (the group's arrival barrier already waits for those;
 *     out-of-group targets are valid and compose)
...
function validateScore(graph, frameNames, subLoader) {
  ...
  // The build rewrites links and the runtime reads markup with double-quoted
  // attributes only, so a single-quoted one would be silently dropped.
  const addSingleQuoted = (quoteGraph, score = null) => {
    for (const [frame, node] of Object.entries(
      (quoteGraph && quoteGraph.byFrame) || {},
    )) {
      for (const attr of node.singleQuotedAttrs || []) {
        errors.push({
          code: "attr-single-quoted",
          frame,
          ...(score ? { score } : {}),
          message: `${attr} on ${frame}${score ? ` in sub-score "${score}"` : ""} uses single quotes — write it as ${attr}="…" (single-quoted attributes are ignored)`,
        });
      }
    }
  };
  ...
  const loadSub = (score) => {
    if (!loadedSubs.has(score)) {
      ...
      if (sub) {
        ...
        addSingleQuoted(sub.graph, score);
      }
    }
  ...
  // 2) Split N is a whole number, and N == href count. A malformed count
  // ("2.5", "2x", "oops", "") never reaches graph.splits, so the runtime would
  // treat the frame as an ordinary one; report it from the raw declaration.
  for (const [frame, node] of Object.entries(graph.byFrame || {})) {
    if (node.splitRaw != null && node.split == null) {
      errors.push({
        code: "split-malformed",
        frame,
        message: `session-split="${node.splitRaw}" on ${frame} is not a whole number — declare the number of child lines, e.g. session-split="2"`,
      });
    }
  }
  ...
  for (const [frame, targets] of Object.entries(graph.holdUntilTargets || {})) {
    // Track-group × hold-until: a grouped frame already waits for the group's
    // other frames via the ARRIVAL barrier, so hold-until may only name frames
    // OUTSIDE the frame's own group (the waits compose).
  ...
  // (owner-clarified), so every target must be one of the frame's outgoing
  // links — a merge frame the line cannot step to is a broken score.
  ...
  for (const [frame, links] of Object.entries(graph.subLinks || {})) {
    ...
    // Ambiguous dive targets. The runtime routes an advance by TARGET FRAME
    // (subLinkForAdvanceTarget in bin/www) — a design older than Session Lines,
    // which never sees WHICH <a> was tapped. So two links on one frame sharing a
    // target are indistinguishable: if either is a dive, every advance to that
    // target dives, including a tap on the plain link; if both dive, `find`
    // takes the first and the second sub-score is unreachable. Both misroute
    // silently in the room, so they are refused here.
    const frameNode = (graph.byFrame || {})[frame] || {};
    const targetCounts = {};
    for (const href of frameNode.hrefs || []) {
      const key = String(href).toLowerCase();
      targetCounts[key] = (targetCounts[key] || 0) + 1;
    }
    const reportedTargets = new Set();
    for (const info of links) {
      const key = String(info.returnHref || "").toLowerCase();
      if (!key || (targetCounts[key] || 0) < 2 || reportedTargets.has(key)) {
        continue;
      }
      reportedTargets.add(key);
      errors.push({
        code: "ambiguous-sub-link-target",
        frame,
        message: `frame ${frame} has ${targetCounts[key]} links pointing at "${info.returnHref}", at least one of them a session-sub-start dive into "${info.score}" — the runtime routes by target frame and cannot tell them apart, so EVERY advance to that target would dive. Give the dive link a target frame of its own.`,
      });
    }
