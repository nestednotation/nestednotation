...
const LINE_FIELDS = [
  ...
  "splitAncestors",
  // When the room last LEFT this line (attrition finalized it dormant), or
  // null for a line nobody was ever on — an empty split branch, a merge undo's
  // unclaimed route. Read only when the room has no active line at all: the
  // joiner then revives the line the room was most recently playing
  // (routing.lastVacatedLineId), not whichever husk has the lowest number.
  "dormantAt",
];
...
class BMLine {
  ...
  setCurrIdxTo(index) {
    // The last boundary before the playhead and its undo trail take a value.
    // Callers validate their own destinations (a tap against the frame's
    // links, a rewind against the saved trail), but the cost of one getting
    // through is out of all proportion to the check: `parseInt` yields NaN for
    // anything non-numeric, `activeFrameList()[index]` is then `undefined`,
    // and the line comes to rest on a frame no score has — addressable by
    // nothing, advanceable from nothing, for the rest of the session. Refusing
    // leaves the line exactly where it was.
    //
    // Converted strictly, not with `Number`: `Number(null)`, `Number("")` and
    // `Number(false)` are all 0, so the loosest conversion would turn three
    // kinds of "no answer" into "go back to the first frame of the score" — a
    // silent move rather than a refused one. A numeric string is still
    // accepted; saved state files carry indexes that way.
    const idx =
      typeof index === "number"
        ? index
    ...
    if (!Number.isInteger(idx) || idx < 0 || idx >= frames.length) {
      console.warn(
        `setCurrIdxTo: line ${this.id} refused out-of-range index ` +
          `${String(index)} (frame list has ${frames.length})`,
      );
      return;
    }
...
/**
 * Has the line's holding period at the frame it is STANDING on run its course?
 *
 * The `isHolding` flag is not the answer, because it is a state and not a clock
 * (owner: "based on the target intrinsic holding period instead of general
 * holding state, to prevent holding block between hold-until notes"). It is set
 * room-wide by the SM hold toggle, with no timer behind it; it survives a state
 * file saved mid-hold, again with no timer; and either way nothing is ever
 * going to clear it. A gate that reads the flag alone then waits for an edge
 * that cannot come — and since a hold-until is exactly such a gate, two frames
 * naming each other stall the room between them.
 *
 * So the question is asked of the FRAME and the clock: the frame's authored
 * `holding` value decided the period (recorded in `currentEndHoldTimeStamp`
 * when it began), and the clock says whether it is over. Only the two states
 * that mean "not begun / still running" hold it open: the 1 s standby gap ahead
 * of the hold, and a live holding timer.
 *
 * Lives here rather than in the runtime so the barrier and group tests can
 * IMPORT the rule they are testing instead of keeping a copy of it — the copies
 * drifted (`isHolding || standbyTimer != null`, which calls a line with a spent
 * deadline and no timer pending when production calls it done) and no fixture
 * happened to tell them apart.
 *
 * @param {object} line
 * @param {function(): number} [now] injectable clock, for tests
 */
function landingHoldElapsed(line, now = Date.now) {
  if (!line) return true;
  if (line.isStandby || line.standbyTimer != null) return false; // not begun
  if (line.holdingTimer != null) return false; // running, it will end itself
  if (!line.isHolding) return true; // nothing is holding this line
  const end = Number(line.currentEndHoldTimeStamp) || 0;
  return end > 0 && now() >= end; // flagged, but the period is spent
}

/** The same question, the way a barrier asks it: is anything still to play? */
function holdPendingAtFrame(line, now = Date.now) {
  return !landingHoldElapsed(line, now);
}
