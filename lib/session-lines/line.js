...
class BMLine {
  ...
  // on when the sub ends. The sub runs with its own (fresh) history; the caller
  // then seeds the playhead at the sub START via setCurrIdxTo.
  ...
  // Pop the current sub-session, returning its { score, returnHref } entry. The
  // caller sets the playhead to the return landing in the main flow.
  exitSub() {
