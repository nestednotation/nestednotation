...
function registryClaimants(session) {
  ...
  for (const line of (session && session.lines) || []) {
    if (line && line.status !== "retired") addTrail(line);
  }
  const standing = (ev) =>
  ...
  for (const ref of (session && session.reachedGranted) || []) add(ref);
  return out;
}
 ...
 * revived line abandons its frozen (stale) position and fast-forwards to the
 * latest track group the room has reached — landing on that group's
 * least-occupied frame, so revivals fill missing track slots instead of
 * doubling covered ones (ties → the recorded arrival frame, then group
 * declaration order).
 *
 ...
 * not do — could only be bought by taking the rejoin off. The map was already
 * drawing that frame as the merged line's live green history while it said so
 * (`survivorGhostRoute` ghosts the route INTO a rejoin, never the rejoin frame
 * itself), which is the same answer from the other direction.
 */
