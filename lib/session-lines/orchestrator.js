...
/**
 * Compute which of a barrier's targets are covered by the histories of the lines
 * currently parked at it. A target is covered when some parked line travelled
 * through that frame — i.e. the lines have actually CONVERGED, not merely that
 * the frame was visited once anywhere.
 *
 * @param {Array<{history?: string[]}>} parkedLines
 * @param {string[]} targets
 * @returns {Set<string>} the subset of `targets` (original casing) that is covered
 */
function barrierCoveredTargets(parkedLines, targets) {
  const covered = new Set();
  const lines = Array.isArray(parkedLines) ? parkedLines : [];
  const histories = lines.map((l) => new Set((l.history || []).map(lc)));
  for (const target of targets || []) {
    if (histories.some((h) => h.has(lc(target)))) {
      covered.add(target);
    }
  }
  return covered;
 ...
 * winner regardless of its local stay tally (decision #3). Returns null if the
 * line has no link votes (it then stays).
 */
