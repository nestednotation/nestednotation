 ...
 * that trail, if the route its forks recorded does (`groupOccurrenceForLine`,
 * 2026-09-12) — the same test that places it there.
 *
 ...
 *
 * @returns {{frame: string, eventId: string, index: number}|null}
 */
...
/**
 * Where one line stands in a track group's history: its own trail if it reaches
 * the group, else the route its forks recorded (2026-09-12).
 *
...
function groupOccurrenceForLine({ line, frames, splitEvents }) {
  ...
  return hit
    ? { frame: hit.frame, index: hit.index, source: "ancestor" }
    : null;
}
 ...
 *
 * A line lands where it WAS: its own trail first, then — when a split truncated
 * that trail past the group — the route its `splitAncestors` record
 * (`ancestorGroupOccurrence`, 2026-09-12). Only a line with neither takes the
 * least-occupied group frame, which is a guess and was being asked to cover a
 * case the records answer exactly. Both of the landings a merge undo gives back
 * mid-rewind are that case: their trails begin at the branch frames of the fork
 * that made them, so a room rewound to the group its lines forked AFTER put
 * every restored line on whichever frame the fill reached first — two lines and
 * four players onto `G` while `H`, the frame one of those routes actually came
 * through, took the empty line.
 *
 * @param {object} opts
...
/**
 * Every structural event of the session in the order it happened, oldest first.
 *
 * Splits and merges live in two append-only arrays with their own id counters,
 * so neither array alone says what happened between two events of the other
 * kind — and a cascade undo needs exactly that. `seq`, allocated from one
 * session-wide counter, is that single ordering; events recorded before it
 * existed fall back to `createdAt` and then to the order they were appended in.
 */
 ...
 * order the runtime walks (owner, 2026-09-06: "able to rewind back to multiple
 * merge points later still possible, not losing the route path").
 *
...
function chainSplitsCanUndo({ chain, mergeEvents }) {
  ...
  const chainMergeIds = new Set(
    (chain || [])
      .filter((entry) => entry && entry.kind === "merge")
      .map((entry) => String(entry.event.id)),
  );
  const merges = Array.isArray(mergeEvents) ? mergeEvents : [];
  return steps.every((entry) => {
    if (!entry.event.blockedByMerge) {
      return true;
    }
    const blockers = merges.filter(
      (merge) =>
        merge &&
        merge.status !== "undone" &&
        (merge.blockedSplitEventIds || [])
          .map(String)
          .includes(String(entry.event.id)),
    );
    return blockers.every(
      (merge) =>
        merge.status === "active" && chainMergeIds.has(String(merge.id)),
    );
  });
}
...
function splitStepState({ event, splitEvents, mergeEvents, lines }) {
  ...
  // walk will undo — an expired one can never be undone, so its mixing is
  // final (`rewindMergeStructure` applies the same rule when it clears the
  // mark).
  const chainMergeIds = new Set(
    chain.chain
      .filter((entry) => entry.kind === "merge")
      .map((entry) => String(entry.event.id)),
  );
  const blockers = (Array.isArray(mergeEvents) ? mergeEvents : []).filter(
    (merge) =>
      merge &&
      merge.status !== "undone" &&
      (merge.blockedSplitEventIds || [])
  ...
  // …and the walk has to be POSSIBLE at all (2026-09-09). `plan.available`
  // is `splitRewindPlan`'s answer about this one step, and it never sees
  // the chain — so a split whose own collapse is fine but which has an
  // EXPIRED event standing on it was published as available, and the click
  // then bought `blocked-by-expired` from `undoStructuralChain`. That is
  // the shape this menu spent 2026-09-07 learning not to have: a button
  // that always refuses is worse than a note saying why there is none, and
  // after a room checkpoint rewind it was the ONLY affordance left on the
  // fork, so the operator's last apparent route back was a dead one. The
  // merge side has always gated on this (`mergeRewindPlan` tests the chain
  // before anything else); this is the same rule, one event kind later.
  //
  // The cascade's own MERGE steps have to be undoable too, for the reason
  // its split steps do (`chainMergesCanUndo`, 2026-09-12).
...
function splitRewindOptions({ splitEvents, mergeEvents, lines }) {
  return (Array.isArray(splitEvents) ? splitEvents : [])
    ...
    .map((event) => {
      ...
      // an expired rejoin has always had, and the fork never did (2026-09-12).
      // Nothing expires at runtime any more; what does expire one is the uid
      // migration on load (`migrateStructuralIdentities`), when a saved room's
      // records cannot be tied back to its lines. That left the fork's node
      // with an empty menu, which is the one shape this menu spent 2026-09-07
      // and 2026-09-09 learning not to have: indistinguishable from a bug, and
      // silent about the only thing the operator needs to know.
      ...
      // ONE RELEASE, ONE UNDO (2026-09-12) — see `splitGestureEvents`. Every
      // member keeps its own node and its own button, and each of them undoes
      // the whole release, so the question the operator answers is the gesture
      // they actually watched happen.
 ...
 * anything the room built on top of this merge is still standing
 * (`superseded` — a later split or merge on the same population). The cascade
 * lives one level up: `structuralRewindChain` names those later events, the
 * runtime undoes them newest-first, and each one lands here as an ordinary
 * fully-validated step. A merge therefore stays undoable for the rest of the
 * session (owner, 2026-09-06) instead of dying at the room's next structural
 * step; only a room checkpoint rewind still `expire`s one, because it
 * repositions the whole population with nothing describing where it stood.
 */
 ...
 * 2026-09-13). The node a co-presence rejoin happened on offered
 * "⏪⏪ rewind L0 here — out of the merge at ⟨B-prime⟩" beside
 * "⏪⏪ undo the merge at ⟨B-prime⟩", so putting the line back on that frame —
 * the one thing the second button does not do — could only be bought by taking
 * the rejoin off. The map was already drawing that frame as the merged line's
 * live green history while it said so (`survivorGhostRoute` ghosts the route
 * INTO a rejoin, never the rejoin frame itself), which is the same answer from
 * the other direction.
 */
 ...
 *
 * Lines converging on a rejoin frame merge in PAIRS: `applyRecombine` runs once
 * per arrival, so three lines meeting at `H` are two events. The operator
 * watched ONE thing happen, so undoing it is ONE gesture (owner, 2026-09-05):
 * every line that met here goes back to the node it came from, and the pairing
 * stays what it always was — the record of how the room got there, not a queue
 * of separate undos to click through. `mergeRewindPlan` still refuses any order
 ...
 * numbers recycle and two unrelated lines can both be "L0"), same frame, and
 * the survivor never left the rejoin point in between (`stayedAtRejoin`).
...
function mergeRewindOptions({ mergeEvents, splitEvents, lines }) {
  return (Array.isArray(mergeEvents) ? mergeEvents : [])
    ...
    .map((event) => {
      ...
      // first (2026-09-06). The chain is what the runtime will actually walk,
      // so the menu can say so before the operator commits to it — and the
      // plan needs the same answer, so it is walked ONCE and handed down.
      const chain = structuralRewindChain({
        session: { splitEvents, mergeEvents },
        kind: "merge",
        eventId: event.id,
      ...
      // (`chainMergesCanUndo`, 2026-09-12): `separable` asks this of THIS
      // event and never asked it of the steps the walk goes through.
      ...
      // By NUMBER (2026-09-09), because the confirm the button raises names the
      // same set that way (`mergeRestoreLandings`): merge order put the last
      // line to arrive in the middle, so one gesture read `L0, L2, L1` on the
      // button and `L0 → …, L1 → …, L2 → …` one click later, and the unsorted
      // half looks like a fault to whoever is reading it mid-piece.
      const restoredIds = [];
 ...
 * Walking it back there as one line would put devices that arrived from another
 * route onto a frame that route never touched — a room state the score never
 * produced — so `lineRewind` undoes these first (owner, 2026-09-05).
 *
 ...
 * so it answers to the same rule (owner, 2026-09-05: "why don't we undo merge
 * automatically?"). Sending every line back to a checkpoint BEHIND a rejoin
 * without undoing the rejoin loses the lines it swallowed: the room stands at a
 * barrier it once crossed with three lines and only two are there, because the
 * third was absorbed after it and nothing brought it back.
 *
 ...
 *
 * An ANCESTRY landing measures from zero (2026-09-12). The frame comes from the
 * route a fork recorded, which is a moment BEFORE this line's own trail begins —
 * so every rejoin the line survived happened after the checkpoint and every one
 * of them comes off. Leaving them standing would be the same loss this function
 * exists to prevent, one fork further back: the room would return to a group it
 * crossed with three lines and find two, precisely for the lines a split's
 * truncated trail made eligible in the first place.
 *
 ...
 *
 * A line that was mid-DIVE keeps its snapshot position: its trail is the SUB's,
 * and the frame list this resolves against is the main flow's.
 *
...
function preMergeLanding({ snapshot, mergeFrame, frameIndexOf }) {
  ...
  const index = frameIndexOf(history[i]);
  if (index < 0) {
 ...
 * they are SPREAD instead (owner, 2026-09-06), the same balancing a split gives
 * the members it finds unassigned, so the undo leaves the room as evenly
 * populated as the moment it is undoing.
 *
 * Split out of `rewindMergeStructure` (2026-09-09) because a CONVERGENCE undoes
 * in PAIRS: `applyRecombine` runs once per arrival, so three lines meeting on
 * one frame are two events, and balancing inside each event in turn spends the
...
function spreadMergeLatecomers({
  ...
  const isPopulation = (conn) => !conn.isMapView && conn.isStaff !== false;
...
function rewindMergeStructure({
  ...
  const lowerFrames = (
    ...
    (session.listFiles || []).map(lc)
  ).map(lc);
  const frameIndexOf = (name) => lowerFrames.indexOf(lc(name || ""));
  ...
  for (const other of session.mergeEvents || []) {
    if (!other || other === plan.event || other.status === "undone") {
      continue;
    }
    for (const blockedId of other.blockedSplitEventIds || []) {
      stillBlocked.add(String(blockedId));
    }
  }
  for (const blockedId of plan.event.blockedSplitEventIds || []) {
    if (stillBlocked.has(String(blockedId))) {
      continue;
    }
...
function createOrchestrator(transport) {
  ...
  function applySplit({
    ...
    // line (2026-09-06): the merge stays recorded, and reaching back for it
    // undoes THIS split on the way (`structuralRewindChain`). Nothing has to be
    // freed here either — an absorbed line kept no number to give back.
    ...
    // A branch NOBODY took is born dormant (2026-09-12).
    //
  ...
  function applyRecombine({ session, lineIds, connections, frame }) {
    ...
    const participants = [...new Set(lineIds || [])]
      ...
      .filter(Boolean);
    // Structural merge record (2026-09-05): a merge is the one event that
    // DESTROYS lines, so it snapshots what it is about to swallow — each
    // participant's frame, trail, dive and devices — and `rewindMergeStructure`
    // puts exactly that back. Recorded BEFORE any field is touched: the
    // survivor's ancestry is narrowed and its playhead moved to the rejoin
    // frame right after this. A "merge" of ONE line (the barrier release path
    // calls this for a lone parked line) absorbs nothing, so it records
    // nothing: there would be nothing to separate again.
    ...
    const deviceIdsFor = (lineId) => {
      ...
      for (const conn of connections || []) {
        // Map tabs are observation tools: they follow the survivor and are not
        // restored to a line of their own (they leave no footprint anywhere).
        if (
          conn &&
          conn.lineId === lineId &&
          conn.deviceId != null &&
    ...
    // number goes straight back in the pool (2026-09-06). The merge undo does
    // not need the object: it re-creates the line from the snapshot it just
    // took, which is what lets the rejoin stay undoable all evening without the
    // room's numbering climbing.
    pruneRetiredLines(session);
