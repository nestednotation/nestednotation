...
- **History-modal checkpoint UX** (reading (i), decided 2026-07-07): when 2+ populated lines are diverged, the modal should offer the last synchronized track group as a room-wide rewind target instead of disabling. Server groundwork (jump = new reached generation + un-park) is in; the client modal + room-wide rewind operation are not.
