...
- **Build:** make `startVotingForSessionWithDelay`/`stopVotingForSession`/`startHoldingForSessionWithDelay` + timers, the `MSG_TAP` path ([bin/www:530](../bin/www#L530)), `jumpScoreForSession`, `cancel*Timer`, and the `MSG_NEED_DISPLAY` reply all take `(session, line)` and read/write `line.*` / target `line.id`. Single line = `lines[0]` ⇒ identical. Shim now only backs persistence-field access. Session Lines also keeps a window-local `_lastVoteCounts`; validates votes against `stay` or the current frame's graph-derived outgoing vote ids (ignoring stale/malformed link ids); and, for non-split lines that close with no valid votes and no retained tally, uses `defaultOutgoingVoteId` over the current frame's ordered `frameLinks` to synthesize a link winner. One outgoing link is taken directly, multiple valid outgoing links are random-tied, and no valid outgoing link leaves the line where it is; explicit `stay` still wins normally.
...
- **Build:** transient `session._barrier` (satisfied set + rejoin arrivals). On hold-until arrival → park + `MSG_BARRIER_WAITING`; mark targets satisfied as lines reach them; all satisfied → `MSG_BARRIER_RELEASED` + proceed. If a missing target has no active, device-bearing, unparked line that can still reach it (main graph or qualified sub graph), it no longer holds the barrier open; the barrier releases once no still-missing target remains reachable. Rejoin: converging lines merge (`planRecombine`), reassign devices, **hard-retire** absorbed lines. **Attrition → 0 devices sets the line dormant: release barriers depending on it (#11), keep it revivable (#13); split-parent / rejoin-absorbed lines hard-retire instead.** Client: barrier-waiting banner (`#barrier-waiting-indicator` + body class).
...
- **Build:** when a line in group G starts voting, open synchronized windows on all **votable** co-group lines (initiator's duration). At group window close, compute the single global-max count across all grouped lines' tallies; if it's a **stay** → set every grouped line's winner to stay (all stay); else each line resolves its own link winner. A line with no local link winner after the global stay check uses the same default outgoing-link fallback as normal voting. Set each line's `winningVoteId` accordingly — the client glow already follows it ([voting.js:111](../public/javascripts/voting.js#L111), [voting.js:201](../public/javascripts/voting.js#L201)), **no client change**. Reuse the random tiebreak ([bin/www:236](../bin/www#L236)).
- **Split ∩ track-group (S7, decided 2026-07-17):** a grouped `session-split` frame still **divides** its line when the group advances — `resolveGroupVoting` delegates it to `resolveSplit` (choosers to their tapped child, stragglers balanced) instead of the whole-line link-winner/default path; a global **stay** brakes it un-split like any other grouped line. `stopVotingForSession` checks the group **before** the split so the outcome no longer depends on whose synchronized window closes first.
- **Verify:** orchestrator test (group window + global-stay propagation + no-local-link fallback); grouped-split runtime test on "-test- Session lines 2" (G votes, H's 2 no-vote players balance 1/1 across J/K); fixture multi-tab — grouped lines vote together; strongest stay anywhere brakes all; a no-local-link grouped line auto-continues when global stay does not win.
