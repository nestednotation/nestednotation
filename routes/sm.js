...
router.get('/', function(req, res, next) {
    ...
    if (permission != '2' || checkExist == null || checkExist.password.trim() != password.trim()){
    ...
    if (req.url != '/'){
        ...
        if (index != null &&
            ...
            holdDur != null && holdDur.length > 0 &&
            voteDur != null && voteDur.length > 0 &&
            adminpassword != null && adminpassword.length > 0 &&
            playerpassword != null && playerpassword.length > 0 &&
            command != null){
                if (command == 'u'){
                    //hold duration
                    ...
                    if (listScore.indexOf(folder) >=0 ){
                        ...
                        var session = db.session.add(ownerId, folder, name, adminpassword, playerpassword);
                        var hold = parseInt(holdDur);
                        if (hold >= 0){
                            session.holdDuration = hold;
                        }
